...
summarize_deployment_by_state_sector_year = function(df, runpath, scen_name, by_tech = F){
  ...
  data = collect(summarise(g, nat_installed_capacity_gw  = sum(installed_capacity)/1e6, 
                           ...
                           nat_market_share = sum(number_of_adopters)/sum(buildings_in_bin), 
                           nat_max_market_share = sum(max_market_share * buildings_in_bin)/sum(buildings_in_bin),
...
diffusion_trends_du<-function(df, df_capacity, runpath, scen_name, by_tech = F, save_results = T){
  
  # df = group_by(df, year, tech) %>%
  #   arrange(year) %>%
  #   mutate(nat_number_of_adopters = sum(new_adopters)) %>%
  #   # mutate(test = cumsum(new_adopters)) %>%
  #   collect()
  ...
  # Convert MW to GW
  df_capacity = mutate(df_capacity, cumulative_market_share_gw = cumulative_market_share_mw/1E3)
  df_capacity = mutate(df_capacity, new_incremental_capacity_gw = new_incremental_capacity_mw/1E3)
  
  # National market share trends
  baseline_data = collect(filter(df_capacity, year==2012))
  baseline_data = baseline_data[,c('year', 'cumulative_market_share_gw')]
  names(baseline_data) = c('year', 'nat_installed_capacity_gw')
  # TODO: Residential sector only used for graphics testing
  baseline_data$sector = 'Residential'
  
  baseline_data = melt(baseline_data, id.vars = melt_vars)
  baseline_data$scenario = scen_name
  baseline_data$data_type = 'Cumulative'
  
  data = collect(filter(df_capacity, year!=2012))
  data = data[,c('year', 'cumulative_market_share_pct', 'cumulative_market_share_gw')]
  names(data) = c('year', 'nat_market_share', 'nat_installed_capacity_gw')
  ...
  yearly_data = collect(filter(df_capacity, year!=2012))
  yearly_data = yearly_data[,c('year', 'new_incremental_capacity_gw')]
  names(yearly_data) = c('year', 'nat_installed_capacity_gw')
  yearly_data$sector = 'Residential'
  ...
  ### ------ ADOPTERS ------ ###
  # Assume # adopters in 2012 was 0
  baseline_adopters = data.frame(year = 2012, nat_number_of_adopters = 0)
  baseline_adopters$sector = 'Residential'
  baseline_adopters = melt(baseline_adopters, id.vars = melt_vars)
  baseline_adopters$scenario = scen_name
  baseline_adopters$data_type = 'Cumulative'
  
  yearly_adopters = group_by(df, year) %>%
    summarise(nat_number_of_adopters = sum(new_adopters)) %>%
    collect()
  cumulative_adopters = yearly_adopters %>%
    arrange(year) %>%
    mutate(nat_number_of_adopters = cumsum(nat_number_of_adopters))
  
  yearly_adopters$sector = 'Residential'
  yearly_adopters = melt(yearly_adopters, id.vars = melt_vars)
  yearly_adopters$scenario = scen_name
  yearly_adopters$data_type = 'Annual'
  
  cumulative_adopters$sector = 'Residential'
  cumulative_adopters = melt(cumulative_adopters, id.vars = melt_vars)
  cumulative_adopters$scenario = scen_name
  cumulative_adopters$data_type = 'Cumulative'

  cumulative_adopter_data = rbind(cumulative_adopters, baseline_adopters)
  combined_adopter_data = rbind(cumulative_adopter_data, yearly_adopters)
  combined_adopter_data$data_type = factor(combined_adopter_data$data_type, levels = c('Cumulative', 'Annual'))
  
  combined_adopter_data$sector = sector2factor(combined_adopter_data$sector)
  cumulative_adopters$sector = sector2factor(cumulative_adopters$sector)
  yearly_adopters$sector = sector2factor(yearly_adopters$sector)
  ...
  # NATIONAL NUMBER OF ADOPTERS
  national_num_of_adopters_bar <- add_data_source_note(
    ggplot(data = subset(combined_adopter_data, variable %in% c("nat_number_of_adopters"))) +
      geom_bar(aes_string(x = 'factor(year)', fill = color_var, weight = 'value')) +
      facet_wrap(~data_type, scales = 'free') +
      scale_color_manual(values = colors) +
      scale_fill_manual(name = simpleCap(color_var), values = colors, guide = guide_legend(reverse=TRUE)) +
