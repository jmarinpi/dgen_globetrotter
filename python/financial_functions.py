...
def cashflow_constructor(bill_savings,
    ...
    if np.size(batt_cost_per_kwh) != n_agents or n_agents == 1: batt_cost_per_kwh = np.repeat(batt_cost_per_kwh,
                                                                                              n_agents)
    ...
    if np.size(down_payment_fraction) != n_agents or n_agents == 1: down_payment_fraction = np.repeat(
        down_payment_fraction, n_agents)
    if np.size(loan_rate_real) != n_agents or n_agents == 1: loan_rate_real = np.repeat(loan_rate_real, n_agents)
    ...
    if np.array(deprec_sched).ndim == 1 or n_agents == 1: deprec_sched = np.array(deprec_sched)
    ...
    loan_rate_nom = (1 + loan_rate_real) * (1 + inflation) - 1

    initial_debt = net_installed_cost - up_front_cost
    annual_principal_and_interest_payment = initial_debt * (loan_rate_nom * (1 + loan_rate_nom) ** loan_term) / ((1 + loan_rate_nom) ** loan_term - 1)
    debt_balance = np.zeros(shape)
    interest_payments = np.zeros(shape)
    principal_and_interest_payments = np.zeros(shape)

    debt_balance[:, :loan_term] = (
                                  initial_debt * ((1 + loan_rate_nom.reshape(n_agents, 1)) ** np.arange(loan_term)).T).T - (
                                  annual_principal_and_interest_payment * (((1 + loan_rate_nom).reshape(n_agents,
                                                                                                    1) ** np.arange(
                                      loan_term) - 1.0) / loan_rate_nom.reshape(n_agents, 1)).T).T
    interest_payments[:, 1:] = (debt_balance[:, :-1].T * loan_rate_nom).T
