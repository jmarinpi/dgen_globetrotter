...
"""                
    ...
    avg_annual_payment = (-annual_loan_pmts.sum(axis = 1)/df.loan_term_yrs) + down_payment_cost.sum(axis = 1)/20 # First argument is for loan payments, second is for downpayment 
    ...
    # If first_year_bill_without_system is zero, percent_mbs will be non-finite
    percent_monthly_bill_savings = np.where(df.first_year_bill_without_system.values, 0, percent_monthly_bill_savings)
