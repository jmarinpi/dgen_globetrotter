...
def calc_diffusion_solar(df, is_first_year, bass_params, 
    ...
    bass_params = bass_params[['control_reg_id','sector_abbr','state', 'p', 'q', 'teq_yr1']]  
    df = pd.merge(df, bass_params, how = 'left', on  = ['control_reg_id','sector_abbr','state'])
    ...
    market_last_year = df[['control_reg_id', 'control_reg', 'state', 'tariff_class', 'country_abbr', 'sector_abbr', 'tech',
...
def calc_diffusion(df, cur, con, techs, choose_tech, sectors, schema, is_first_year,
    ...
    df['market_value'] = df['market_value_last_year'] + df['new_market_value']
    market_last_year = df[['county_id','bin_id', 'sector_abbr', 'tech', 'market_share', 'max_market_share','number_of_adopters', 'installed_capacity', 'market_value', 'initial_number_of_adopters', 'initial_pv_capacity_mw', 'initial_market_share', 'initial_market_value']] # Update dataframe for next solve year
    market_last_year.columns = ['county_id', 'bin_id', 'sector_abbr', 'tech', 'market_share_last_year', 'max_market_share_last_year','number_of_adopters_last_year', 'installed_capacity_last_year', 'market_value_last_year', 'initial_number_of_adopters', 'initial_pv_capacity_mw', 'initial_market_share', 'initial_market_value']
