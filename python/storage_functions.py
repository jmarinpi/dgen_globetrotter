...
def system_size_and_bill_calc_optimal(agent, deprec_sch_df, pv_cf_profile_df, rates_rank_df, rates_json_df):
    ...
    for i in system_df.index:    
        ...
        estimated_results = dFuncs.determine_optimal_dispatch(load_profile, pv_size*pv_cf_profile, batt, tariff, export_tariff, estimator_params=estimator_params, estimated=True, DP_inc=DP_inc, d_inc_n=d_inc_n, restrict_charge_to_pv_gen=True)
...
#%%
def system_size_driver(agent_df, deprec_sch_df, pv_cf_profile_df, rates_rank_df, rates_json_df):  
