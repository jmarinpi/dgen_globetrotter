...
def create_tech_choice_report(choose_tech, schema, scen_name, out_scen_path, cur, con, Rscriblock_path, pg_params_file, file_suffix=''):
    ...
    if choose_tech == True:
        ...
        plot_outputs_path = '%s/r/graphics/tech_choice_report.R' % os.path.dirname(
            os.getcwd())
        ...
        # passed to subprocess
        command = [Rscriblock_path, '--vanilla', plot_outputs_path,
                   out_path, scen_name, schema, pg_params_file, file_suffix]
        proc = subprocess.Popen(
            command, stderr=subprocess.PIPE, stdout=subprocess.PIPE)
        messages = proc.communicate()
        if 'error' in messages[1].lower():
        ...
        logger.info(
            "\tSkipping Creation of Technology Choice Report (Not Applicable)")
...
def summarize_scenario(scenario_settings, model_settings):
    ...
    logger.info('\tTech Choice Mode Enabled: %s' %
                scenario_settings.choose_tech)
