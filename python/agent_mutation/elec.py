...
def aggregate_outputs_solar(agent_df, year, is_first_year,
    ...
    '''aggregate_outputs_solar
        ...
        Inputs:
        -agent_df - pandas dataframe - agent attributes for the given year
        -year - int - modelled year
        -is_first_year - bool - is first year indicator
        -scenario_settings - object - scenario settings loaded from input sheet and csv's
        -interyear_results_aggregations - pandas dataframe - aggregated pandas dataframe from previous year
        
        Outputs:
        -interyear_results_aggregations - pandas dataframe - aggregated agent attributes
    '''
...
def apply_elec_price_multiplier_and_escalator(dataframe, year, elec_price_change_traj):
    '''apply_elec_price_multiplier_and_escalator
        ...
        Inputs:
        -dataframe - pandas dataframe - agent attributes for the given year
        -year - int - modelled year
        -elec_price_change_traj - pandas dataframe - contains elec_price_multiplier field by country, control region, and sector
        
        Outputs:
        -dataframe - pandas dataframe - agent attributes with new attributes
    '''
...
def apply_export_tariff_params(dataframe, net_metering_df):
    '''apply_export_tariff_params
        ...
        -dataframe - pandas dataframe - agent attributes for the given year
        -year - int - modelled year
        -net_metering_df - pandas dataframe - net metering system size limitations including nem_system_size_limit_kw,year_end_excess_sell_rate_usd_per_kwh,hourly_excess_sell_rate_usd_per_kwh
...
def apply_wholesale_elec_prices(dataframe, df):
    '''apply_wholesale_elec_prices
        ...
        Inputs:
        -dataframe - pandas dataframe - agent attributes for the given year
        -df - pandas dataframe - includes joinable wholesale_elec_usd_per_kwh field
...
def apply_pv_specs(dataframe, pv_specs):
    '''apply_pv_specs
        ...
        Inputs:
        -dataframe - pandas dataframe - agent attributes for the given year
        -pv_specs - pandas dataframe - contains joinable fields for pv_power_density_w_per_sqft,pv_deg,pv_price_per_kw,pv_om_per_kw,pv_variable_om_per_kw
...
def apply_storage_specs(dataframe, batt_price_traj, year, scenario_settings):
    '''apply_storage_specs
        ...
        -dataframe - pandas dataframe - agent attributes for the given year
        -year - int - modelled year
        -scenario_settings - object - scenario settings loaded from input sheet and csv's, with battery replacement year and replacement fractions by kw and kwh
        -batt_price_traj - pandas dataframe - contains joinable fields batt_price_per_kwh,batt_price_per_kw,batt_om_per_kw,batt_om_per_kwh
...
def apply_financial_params(dataframe, financing_terms, inflation_rate):
    '''apply_financial_params
        ...
        Inputs:
        -dataframe - pandas dataframe - agent attributes for the given year
        -inflation_rate - float - inflation rate percent
        -financing_terms - pandas dataframe - contains joinable fields: deprec_sch,itc_fraction,min_size_kw,max_size_kw,loan_term,loan_rate,down_payment,real_discount,tax_rate,economic_lifetime 
...
def apply_load_growth(dataframe, load_growth_df):
    '''apply_load_growth
        ...
        Inputs:
        -dataframe - pandas dataframe - agent attributes for the given year
        -load_growth_df - pandas dataframe - contains joinable load_multiplier field
...
def apply_carbon_intensities(dataframe, carbon_intensities):
    '''apply_carbon_intensities
        ...
        Inputs:
        -dataframe - pandas dataframe - agent attributes for the given year
        -carbon_intensities - pandas dataframe - contains joinable carbon_price_cents_per_kwh field
...
def apply_market_last_year(dataframe, market_last_year_df):
    '''estimate_initial_market_shares
        ...
        Inputs:
        -dataframe - pandas dataframe - agent attributes for the given year
        -market_last_year_df - pandas dataframe -last year's number of adopters and percent adoption at the state level
