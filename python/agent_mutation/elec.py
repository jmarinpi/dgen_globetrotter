...
def apply_elec_price_multiplier_and_escalator_simple(dataframe, year, rate_growth_df):
    ...
    rate_growth_df['elec_price_escalator'] = (rate_growth_df[str(
        year)] / rate_growth_df['2010'])**(1.0 / (year - 2010)) - 1.0

    dataframe = pd.merge(dataframe, rate_growth_df[
                         ['census_division_abbr', 'elec_price_multiplier', 'elec_price_escalator']], how='left', on=['census_division_abbr'])
...
def apply_tech_performance_solar(dataframe, tech_performance_solar_df):
    ...
    dataframe = pd.merge(dataframe, tech_performance_solar_df[
...
def apply_financial_params(dataframe, financial_params_df, itc_options, tech_costs_solar_df):
    ...
    # probably would rather not have this included as a column
    dataframe['inflation'] = 0.025
...
def get_sam_electric_rates(cur, con, schema, sectors, seed, pg_conn_string, mode):
    ...
    if mode == 'develop':
        ...
        inputs['excluded_rate_ids'] = '(' + ', '.join([str(i[0])
                                                       for i in excluded_rates.values]) + ')'
        ...
        for sector_abbr, sector in sectors.iteritems():
            ...
            if rate_structure.lower() == 'complex rates':
                ...
                # we simply need to duplicate and rename that field to
                # rate_id_alias and specify the rate_source
                sql = """DROP TABLE IF EXISTS %(schema)s.agent_electric_rate_tariffs_lkup_%(sector_abbr)s;
                         CREATE UNLOGGED TABLE %(schema)s.agent_electric_rate_tariffs_lkup_%(sector_abbr)s AS
...
def calculate_excess_generation_and_update_nem_settings(dataframe, gross_fit_mode=False):
    ...
    excess_gen_hourly = np.maximum(
        gen_array - np.array(list(dataframe[con])), 0)
    ...
    with np.errstate(invalid='ignore'):
        ...
        dataframe['excess_generation_percent'] = np.where(
            annual_generation == 0, 0, excess_gen_annual / annual_generation)
    ...
    if gross_fit_mode == True:
        ...
        # no excess generation will be credited at the flat sell rate (outside
        # of SAM)
        ...
        # otherwise, if full net metering is not availabl,e all excess
        # generation will be credited at the flat sell rate
        dataframe['flat_rate_excess_gen_kwh'] = np.where(
            dataframe['full_net_metering'] == True, 0, excess_gen_annual)
        # if there is no net metering, calculate the non-excess portion of hourly generation and re-assign it to the gen column
        # this allows us to credit the non-excess (i.e., offsetting) portion of
        # generation at full retail rates
        dataframe[gen] = np.where(dataframe['full_net_metering'] == True, dataframe[
                                  gen], pd.Series(offset_generation))
        # set SAM to run net metering calcs
        dataframe['ur_enable_net_metering'] = True

    # calculate the value of the excess generation (only applies to NET FIT
    # when full net metering is not available)
    dataframe['net_fit_credit_dollars'] = dataframe[
        'flat_rate_excess_gen_kwh'] * dataframe['ur_flat_sell_rate']
...
def calculate_electric_bills_sam(dataframe, n_workers):
    ...
    in_cols = list(dataframe.columns)
    # add a new unique identifier field for each row (for backwards
    # compatibility with the way pssc_mp was set up)
    ...
    # adjust the first_year_bill_with_system to account for the
    # net_fit_credit_dollars
    dataframe['first_year_bill_with_system'] = dataframe[
        'elec_cost_with_system_year1'] - dataframe['net_fit_credit_dollars']
    # rename elec_cost_without_system_year1 to first_year_bill_without_system
    dataframe['first_year_bill_without_system'] = dataframe[
    ...
    out_cols = ['first_year_bill_with_system',
                'first_year_bill_without_system', 'cost_of_elec_dols_per_kwh']
