...
def cashflow_constructor(bill_savings,
    ...
    after_tax_bill_savings = (bill_savings.T * (1 - (
    sector != 'res') * effective_tax_rate)).T  # reduce value of savings because they could have otherwise be written off as operating expenses
    ...
    annual_principal_and_interest_payment = initial_debt * (loan_rate * (1 + loan_rate) ** loan_term) / (
    ...
    debt_balance[:, :loan_term] = (
                                  initial_debt * ((1 + loan_rate.reshape(n_agents, 1)) ** np.arange(loan_term)).T).T - (
                                  annual_principal_and_interest_payment * (((1 + loan_rate).reshape(n_agents,
                                                                                                    1) ** np.arange(
                                      loan_term) - 1.0) / loan_rate.reshape(n_agents, 1)).T).T
