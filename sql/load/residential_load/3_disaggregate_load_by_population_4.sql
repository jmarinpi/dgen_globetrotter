...
FROM dg_wind.res_customers_by_county_us; -- 1,417,737,929.8571711127352

select sum(total_residential_customers)
FROM dg_wind.ventyx_elec_serv_territories_w_2011_sales_data_backfilled_clip
where state_abbr not in ('AK','HI'); -- 1,417,737,928

select X - Y; -- Z (difference likely due to rounding)
select (X - Y)/X  * 100; -- Z2 % load is missing nationally
