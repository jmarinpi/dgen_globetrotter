-- DROP TABLE IF EXISTS wind_ds_data.pt_grid_us_res;
CREATE TABLE wind_ds_data.pt_grid_us_res (
	x numeric,
	y numeric,
	res_load numeric);
...
COPY wind_ds_data.pt_grid_us_res FROM '/srv/home/mgleason/data/dg_wind/res_load_us_200m_points.txt' with csv header;

RESET ROLE;

ALTER TABLE wind_ds_data.pt_grid_us_res ALTER x TYPE INTEGER;
ALTER TABLE wind_ds_data.pt_grid_us_res ALTER y TYPE INTEGER;

ALTER TABLE wind_ds_data.pt_grid_us_res ADD COLUMN the_geom_900914 geometry;

UPDATE wind_ds_data.pt_grid_us_res
SET the_geom_900914 = ST_SetSRID(ST_MakePoint(x,y),900914);

CREATE INDEX pt_grid_us_res_the_geom_900914_gist ON wind_ds_data.pt_grid_us_res USING gist(the_geom_900914);
CLUSTER wind_ds_data.pt_grid_us_res USING pt_grid_us_res_the_geom_900914_gist;

VACUUM ANALYZE wind_ds_data.pt_grid_us_res;

-- add:
-- gid (serial) and pkey
ALTER TABLE wind_ds_data.pt_grid_us_res ADD COLUMN gid serial;
ALTER TABLE wind_ds_data.pt_grid_us_res ADD PRIMARY KEY (gid);
-- the_geom_4326 with index
ALTER TABLE wind_ds_data.pt_grid_us_res ADD COLUMN the_geom_4326 geometry;

UPDATE wind_ds_data.pt_grid_us_res
SET the_geom_4326 = ST_Transform(the_geom_900914,4326);

CREATE INDEX pt_grid_us_res_the_geom_4326_gist ON wind_ds_data.pt_grid_us_res USING gist(the_geom_4326);
-- county id (from polygons) and index (foreign key)?
ALTER TABLE wind_ds_data.pt_grid_us_res ADD COLUMN county_id integer;

UPDATE wind_ds_data.pt_grid_us_res a
SET county_id = b.county_id
FROM wind_ds.county_geom b
WHERE ST_Intersects(a.the_geom_900914, b.the_geom_900914);

CREATE INDEX pt_grid_us_res_county_id_btree ON wind_ds_data.pt_grid_us_res using btree(county_id);

-- check why there are nulls
CREATE TABLE wind_ds_data.no_county_pts AS
SELECT gid, the_geom_4326
FROM wind_ds_data.pt_grid_us_res
...
FROM wind_ds_data.pt_grid_us_res a
...
CREATE TABLE wind_ds_data.pt_grid_us_res_iiijjjicf_id_lookup (
	...
	SELECT parsel_2('dav-gis','wind_ds_data.pt_grid_us_res','gid',
	...
	ALTER TABLE wind_ds_data.pt_grid_us_res ADD COLUMN iiijjjicf_id integer;
	...
	CREATE INDEX pt_grid_us_res_iiijjjicf_id_btree ON wind_ds_data.pt_grid_us_res USING btree(iiijjjicf_id);
	...
	SELECT a.gid, a.the_geom_900914, 
		(SELECT b.iiijjjicf_id 
		 FROM wind_ds_data.pt_grid_us_res b
...
-- population density only
	...
	-- join the info back in
	ALTER TABLE wind_ds_data.pt_grid_us_res ADD COLUMN maxheight_m_popdens integer;
	...
	-- how many are null?
	CREATE INDEX pt_grid_us_res_maxheight_m_popdens ON wind_ds_data.pt_grid_us_res USING btree(maxheight_m_popdens) where maxheight_m_popdens is null;
	...
	-- isolate the unjoined points
	-- and fix them by assigning value from their nearest neighbor that is not null
	DROP TABLE IF EXISTS wind_ds_data.pt_grid_us_res_maxheight_m_popdens_missing_lookup;
	CREATE TABLE wind_ds_data.pt_grid_us_res_maxheight_m_popdens_missing_lookup AS
	with a AS(
		select gid, the_geom_900914
		FROM wind_ds_data.pt_grid_us_res
		where maxheight_m_popdens is null)
	...
	SET maxheight_m_popdens = b.maxheight_m_popdens
	FROM wind_ds_data.pt_grid_us_res_maxheight_m_popdens_missing_lookup b
	where a.gid = b.gid
...
-- population density and 20 pc canopy cover
	...
	-- join the info back in
	ALTER TABLE wind_ds_data.pt_grid_us_res ADD COLUMN maxheight_m_popdenscancov20pc integer;
	...
	-- how many are null?
	CREATE INDEX pt_grid_us_res_maxheight_m_popdenscancov20pc_btree ON wind_ds_data.pt_grid_us_res USING btree(maxheight_m_popdenscancov20pc) where maxheight_m_popdenscancov20pc is null;
	...
	-- isolate the unjoined points
	-- and fix them by assigning value from their nearest neighbor that is not null
	DROP TABLE IF EXISTS wind_ds_data.pt_grid_us_res_maxheight_popdenscancov20pc_missing_lookup;
	CREATE TABLE wind_ds_data.pt_grid_us_res_maxheight_popdenscancov20pc_missing_lookup AS
	with a AS(
		select gid, the_geom_900914
		FROM wind_ds_data.pt_grid_us_res
		where maxheight_m_popdenscancov20pc is null)
	...
	SET maxheight_m_popdenscancov20pc = b.maxheight_m_popdenscancov20pc
	FROM wind_ds_data.pt_grid_us_res_maxheight_popdenscancov20pc_missing_lookup b
	where a.gid = b.gid
...
-- population density and 40 pc canopy cover
	...
	-- join the info back in
	ALTER TABLE wind_ds_data.pt_grid_us_res ADD COLUMN maxheight_m_popdenscancov40pc integer;
	...
	-- how many are null?
	CREATE INDEX pt_grid_us_res_maxheight_m_popdenscancov40pc_btree ON wind_ds_data.pt_grid_us_res USING btree(maxheight_m_popdenscancov40pc) where maxheight_m_popdenscancov40pc is null;
	...
	-- isolate the unjoined points
	-- and fix them by assigning value from their nearest neighbor that is not null
	DROP TABLE IF EXISTS wind_ds_data.pt_grid_us_res_maxheight_popdenscancov40pc_missing_lookup;
	CREATE TABLE wind_ds_data.pt_grid_us_res_maxheight_popdenscancov40pc_missing_lookup AS
	with a AS(
		select gid, the_geom_900914
		FROM wind_ds_data.pt_grid_us_res
		where maxheight_m_popdenscancov40pc is null)
	...
	SET maxheight_m_popdenscancov40pc = b.maxheight_m_popdenscancov40pc
	FROM wind_ds_data.pt_grid_us_res_maxheight_popdenscancov40pc_missing_lookup b
	where a.gid = b.gid
...
ALTER TABLE wind_ds_data.pt_grid_us_res ADD COLUMN res_rate_gid integer;

UPDATE wind_ds_data.pt_grid_us_res a
SET res_rate_gid = b.gid
FROM wind_ds_data.annual_ave_elec_rates_2011 b
...
-- create dsire incentives lookup table
	...
	CREATE TABLE wind_ds.wind_incentives_lookup AS

	with a as (
	SELECT b.gid, b.the_geom, d.uid as wind_incentives_uid

	FROM dg_wind.incentives_geoms_copy b
	...
	SELECT e.gid as pt_grid_us_res_gid, a.wind_incentives_uid
	FROM a

	INNER JOIN wind_ds_data.pt_grid_us_res e
	ON ST_Intersects(a.the_geom,e.the_geom_4326);

-- test county level access
SELECT county_id,sum(res_rate_gid)
FROM wind_ds_data.pt_grid_us_res
group by county_id;

-- cluster the data on county for faster access
CLUSTER wind_ds_data.pt_grid_us_res USING pt_grid_us_res_county_id_btree;
-- to set it back to gist based clustering
-- CLUSTER wind_ds_data.pt_grid_us_res USING pt_grid_us_res_the_geom_900914_gist;
