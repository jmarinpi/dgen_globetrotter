...
-- utility type
DROP TABLE IF EXISTS wind_ds_data.pt_grid_us_com_utiltype_lookup;
CREATE TABLE wind_ds_data.pt_grid_us_com_utiltype_lookup (
	gid integer,
	...
	FROM isect a
	ORDER BY a.gid, a.rank ASC;','wind_ds_data.pt_grid_us_com_utiltype_lookup', 'a', 16);

	-- join the info back in
	ALTER TABLE wind_ds.pt_grid_us_com ADD COLUMN utility_type character varying(9);

	CREATE INDEX pt_grid_us_com_utiltype_lookup_gid_btree ON wind_ds_data.pt_grid_us_com_utiltype_lookup using btree(gid);

	UPDATE wind_ds.pt_grid_us_com a
	SET utility_type = b.utility_type
	FROM wind_ds_data.pt_grid_us_com_utiltype_lookup b
	where a.gid = b.gid;
	
	CREATE INDEX pt_grid_us_com_utility_type_btree ON wind_ds.pt_grid_us_com USING btree(utility_type);
	...
	DROP TABLE IF EXISTS wind_ds_data.pt_grid_us_com_utiltype_missing;
	CREATE TABLE wind_ds_data.pt_grid_us_com_utiltype_missing AS
	with a AS(
	...
	UPDATE wind_ds.pt_grid_us_com a
	SET utility_type = b.utility_type
	FROM wind_ds_data.pt_grid_us_com_utiltype_missing b
	where a.gid = b.gid
...
-- add pca region
DROP TABLE IF EXISTS wind_ds_data.pt_grid_us_com_pca_reg_lookup;
CREATE TABLE wind_ds_data.pt_grid_us_com_pca_reg_lookup (
	gid integer,
	pca_reg integer,
	reeds_reg integer);

SELECT parsel_2('dav-gis','mgleason','mgleason','wind_ds.pt_grid_us_com','gid',
	...
	'wind_ds_data.pt_grid_us_com_pca_reg_lookup', 'a',16);
...
CREATE INDEX pt_grid_us_com_pca_reg_lookup_gid_btree ON wind_ds_data.pt_grid_us_com_pca_reg_lookup using btree(gid);

UPDATE wind_ds.pt_grid_us_com a
SET (pca_reg,reeds_reg) = (b.pca_reg,b.reeds_reg)
FROM wind_ds_data.pt_grid_us_com_pca_reg_lookup b
where a.gid = b.gid;

-- how many are null?
CREATE INDEX pt_grid_us_com_pca_reg_btree ON wind_ds.pt_grid_us_com USING btree(pca_reg);
CREATE INDEX pt_grid_us_com_reeds_reg_btree ON wind_ds.pt_grid_us_com USING btree(reeds_reg);
...
DROP TABLE IF EXISTS wind_ds_data.pt_grid_us_com_pca_reg_missing_lookup;
CREATE TABLE wind_ds_data.pt_grid_us_com_pca_reg_missing_lookup AS
with a AS(
	select gid, the_geom_900914
	FROM wind_ds.pt_grid_us_com
	where pca_reg is null),
...
from b
LEFT JOIN wind_ds.pt_grid_us_com c
ON b.nn_gid = c.gid;
-- **
...
UPDATE wind_ds.pt_grid_us_com a
SET (pca_reg,reeds_reg) = (b.pca_reg,b.reeds_reg)
FROM wind_ds_data.pt_grid_us_com_pca_reg_missing_lookup b
where a.gid = b.gid
