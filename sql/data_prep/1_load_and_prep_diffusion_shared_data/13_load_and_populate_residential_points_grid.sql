...
SELECT parsel_2('dav-gis','mgleason','mgleason',
		'diffusion_shared.pt_grid_us_res_new',
		'gid',
		'SELECT a.gid, a.the_geom_4326, c.gisjoin as block_gisjoin
		FROM diffusion_shared.pt_grid_us_res_new a
		LEFT JOIN diffusion_shared.county_geom b
			ON a.county_id = b.county_id
		LEFT JOIN census_2010.block_geom_parent c
...
(
	SELECT b.gid,
	(
		SELECT c.block_gisjoin
		 FROM diffusion_wind_data.pt_grid_us_res_new_census_2010_block_lkup c
		 where c.block_gisjoin is not null
		 ORDER BY b.the_geom_4326 <#> c.the_geom_4326
		 LIMIT 1
	 ) as block_gisjoin
	FROM diffusion_wind_data.pt_grid_us_res_new_census_2010_block_lkup b
	where b.block_gisjoin is null
)
UPDATE diffusion_wind_data.pt_grid_us_res_new_census_2010_block_lkup d
SET block_gisjoin = a.block_gisjoin
FROM  a
where d.gid = a.gid
and d.block_gisjoin is null;
