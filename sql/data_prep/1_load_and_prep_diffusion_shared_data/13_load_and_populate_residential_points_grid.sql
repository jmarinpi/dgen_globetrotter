...
UPDATE diffusion_wind_data.pt_grid_us_res_new_census_2010_block_lkup a
SET (housing_units, acres_per_hu) = (b.housing_units,
					CASE WHEN b.housing_units > 0 THEN (a.aland10/4046.86)::numeric/b.housing_units
					   else 100
					   end)		
FROM diffusion_wind_data.census_2010_block_housing_units b
...
UPDATE diffusion_shared.pt_grid_us_res_new a
SET (housing_units, acres_per_hu) = (b.housing_units, b.acres_per_hu)
from diffusion_wind_data.pt_grid_us_res_new_census_2010_block_lkup b
where a.gid = b.gid;
...
-- add an index
CREATE INDEX pt_grid_us_res_new_acres_per_hu_btree
ON diffusion_shared.pt_grid_us_res_new
...
select count(*)
FROM diffusion_shared.pt_grid_us_res_new
-- where acres_per_hu < 3 -- 3602971
-- where acres_per_hu < 1 -- 2423775
where acres_per_hu < .5; -- 1478096

-- add an index on housing_units too
CREATE INDEX pt_grid_us_res_new_housing_units_btree
ON diffusion_shared.pt_grid_us_res_new
using btree(housing_units);

-- how many points are in blocks with zero housing units?
select count(*)
from diffusion_shared.pt_grid_us_res_new
where housing_units = 0; -- only 213895 -- not bad!

-- move these points to a different table
DROP TABLE IF EXIStS diffusion_shared.pt_grid_us_res_new_0hu;
CREATE TABLE diffusion_shared.pt_grid_us_res_new_0hu AS
SELECT *
From diffusion_shared.pt_grid_us_res_new
where housing_units = 0;
...
-- following indices should be kept:
  ...
  -- hi_dev: diffusion_shared.pt_grid_us_res_new_hi_dev_btree
