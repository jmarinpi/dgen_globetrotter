...
select count(*)
FROM diffusion_shared.ranked_urdb_rates_by_pt_com;
------------------------------------------------------------------------------

-- INDUSTRIAL
DROP TABLE IF EXISTS diffusion_shared.ranked_urdb_rates_by_pt_ind;
CREATE TABLE diffusion_shared.ranked_urdb_rates_by_pt_ind
(
	pt_gid integer,
	urdb_rate_id text,
	rank integer,
		...
		'WITH a as
			...
			(
				...
				SELECT pt_gid,  urdb_rate_id, rank() OVER (partition by pt_gid ORDER BY near_utility_type_match desc, distance_m asC) as rank,
					...
					near_utility_type_match::boolean
				FROM c;',
		'diffusion_shared.ranked_urdb_rates_by_pt_ind', 'a', 18);

-- 198913815 rows
-- add indices
CREATE INDEX ranked_urdb_rates_by_pt_ind_pt_gid_btree
ON diffusion_shared.ranked_urdb_rates_by_pt_ind
using btree(pt_gid);

CREATE INDEX ranked_urdb_rates_by_pt_ind_urdb_rate_id_btree
ON diffusion_shared.ranked_urdb_rates_by_pt_ind
using btree(urdb_rate_id);

CREATE INDEX ranked_urdb_rates_by_pt_ind_urdb_rank_btree
ON diffusion_shared.ranked_urdb_rates_by_pt_ind
using btree(rank);

select count(*)
FROM diffusion_shared.ranked_urdb_rates_by_pt_ind;
------------------------------------------------------------------------------

-- RESIDENTIAL
DROP TABLE IF EXISTS diffusion_shared.ranked_urdb_rates_by_pt_res;
CREATE TABLE diffusion_shared.ranked_urdb_rates_by_pt_res
(
	pt_gid integer,
	urdb_rate_id text,
	rank integer,
...
SELECT parsel_2('dav-gis','mgleason','mgleason',
		...
		'WITH a as
			(
				...
				ON b.state_abbr = c.state_abbr
				INNER JOIN diffusion_shared.curated_urdb_rates_res d
				ON c.urdb_rate_id = d.urdb_rate_id
			),
			b as
			(
			...
			(
				...
				SELECT pt_gid,  urdb_rate_id, rank() OVER (partition by pt_gid ORDER BY near_utility_type_match desc, distance_m asC) as rank,
					...
					near_utility_type_match::boolean
				FROM c;',
		'diffusion_shared.ranked_urdb_rates_by_pt_res', 'a', 18);

-- 198913815 rows
-- add indices
CREATE INDEX ranked_urdb_rates_by_pt_res_gid_btree
ON diffusion_shared.ranked_urdb_rates_by_pt_res
using btree(pt_gid);

CREATE INDEX ranked_urdb_rates_by_pt_res_urdb_rate_id_btree
ON diffusion_shared.ranked_urdb_rates_by_pt_res
using btree(urdb_rate_id);

CREATE INDEX ranked_urdb_rates_by_pt_res_urdb_rank_btree
ON diffusion_shared.ranked_urdb_rates_by_pt_res
using btree(rank);

select count(*)
FROM diffusion_shared.ranked_urdb_rates_by_pt_res;