...
ggplot(data = df) +
  geom_point(aes(x = depth_m**.5, y = t_root, colour = t35km))

ggplot(data = df) +
  geom_point(aes(x = depth_m, y = t, colour = t35km))

ggplot(data = df) +
  geom_point(aes(x = t35km**.5, y = t_root, colour = depth_m))

df$t_root = df$t**.5
df$depth_root = df$depth_m**.5
hist(df$t_root)
df$t35km_root = df$t35km**.5
# create regression model
m = lm(t_root ~ t35km_root + depth_root, data = df)
...
# how about residuals and fitted values?
plot(m$residuals ~ m$fitted.values)
# no relationship, but some heteroskedasticity
cor(m$residuals, m$fitted.values) # 0



# # true resids
resids = m$model$t_root**2 - m$fitted.values**2
sd(resids)*2
mean(resids**2)**.5
summary(resids)
hist(resids)
plot(resids ~ m$model$t_root**2)
plot(m$fitted.values**2 ~ m$model$t_root**2)

# how about observed t and actual t
plot(m$residuals ~ m$model$t_root)
