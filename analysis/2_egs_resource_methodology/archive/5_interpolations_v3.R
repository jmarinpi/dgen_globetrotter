...
df2000 = filter(df, slice == 2000) %>%
  group_by(x, y) %>%
  summarize(resid = median(resid),
            t = median(t_c),
            t_pred = median(t_pred)
  ) %>%
  as.data.frame()
sf2000 = df2000
...
results = k2000@data
names(results)[1:2] = c('fitted_resid', 'resid_variance')
results$x = k2000@coords[,1]
results$y = k2000@coords[,2]

pgrid = grid
pgrid$depth_m = 2000
pgrid$fitted_t = predict.lm(m, pgrid)
rgrid = merge(results, pgrid, by = c('x', 'y'))
rgrid$final_t = rgrid$fitted_t + rgrid$fitted_resid

breaks = seq(0, 350, 25)
rgrid$t_c_interval = cut(rgrid$final_t, breaks = breaks, labels = breaks[2:length(breaks)])
...
sgrid = rgrid
sgrid$ci = sqrt(sgrid$resid_variance) * 2
sgrid = filter(sgrid, ci <= 25)

sgrid$t_c_interval = cut(sgrid$final_t, breaks = breaks, labels = breaks[2:length(breaks)])
# df2000$tc_interval = cut(df2000$t, breaks = breaks, labels = breaks[2:length(breaks)])
ggplot(data = sgrid) +
  geom_point(aes(x = x, y = y, colour = t_c_interval)) +
  scale_colour_manual(values = rev(c('#a50026', '#d73027', '#f46d43', '#fdae61', '#fee08b', '#ffffbf', '#d9ef8b', '#a6d96a', '#66bd63', '#1a9850', '#006837'))) +
  geom_point(data = df2000, aes(x = x, y = y), size = 0.5)


ggplot(data = rgrid) +
  geom_point(aes(x = x, y = y, colour = (sqrt(rgrid$resid_variance) * 2) < 25)) +
