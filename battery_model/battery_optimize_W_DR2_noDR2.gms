...
* ----- GAMS Options & Title : -------------------------------------------------
* Dollar control derivatives:  control the appearance and amount of detail in the output
$offlisting
...
* min and max charging capacity of Battery (kWh) *when charging or discharging*
     B_min_OP
     B_max_OP
...
* ----- *need to change to PV data: -----------------------
        PV_gen(T)
*               generation available from solar farm in each hour (per kW)


* ----- Month boundaries, in hours: -----------------------
* ----- from HOURS_SEP_MONTHS below -----------------------
        hrs_MonthL(iM)
        hrs_MonthH(iM)
...
$include .\LOAD_DATA\MODEL_RUN_PARAMETERS_woDR2.dat
...
net_cost_def..
        net_cost =e= sum(T, eBought(T)*ePrice(T))
                  ...
                  -  sum(T, eSold(T)*ePrice(T));

*-------------------------------------------------------------------------------

* --- Sum Demand Costs for each Month and for each Demand time 'bin': -----------------------------
* --- This is finding the max hourly consumption, and then apply demand charges in three blocks ---
Demand_cost1(T,iM)..
...
* --- Set lower and upper bounds on DR charging / discharging: --------------------
DR_discharge_lower_bound(T)..
...
*       Export results
        ...
        put battery_dis;
        put '"Hour", "EnergyPrice", "Load", "Purchases", "Sales", "PV used","PV curtailed", "DR_SOC", "DR_charging", "DR_discharging", "Battery SOC","B_charging","B_discharging", "Discharge", "Charge", "Discharge Start", "Charge Start"'/;
        battery_dis.pc = 5;
        loop(T,
                put ord(T), ePrice(T), eLoad(T), eBought.l(T), eSold.l(T), PVused.l(T), PVcurtailed.l(T), DR_SOC.l(T), DR_charged.l(T), DR_discharged.l(T), Battery_SOC.l(T), Battery_charged.l(T), Battery_discharged.l(T), discharging.l(T), charging.l(T), discharge_start.l(T), charge_start.l(T)/;
        );
);
*===============================================================================
